...
# =================================================================
if [ -f "$STATE" ]; then
  LAST_SHA=$(get_field "$STATE" lastLearnedSha)
  LAST_DATE=$(get_field "$STATE" lastLearnedDate)
  ...
  if command -v git >/dev/null 2>&1 && git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    ...
    # Is the last-learned SHA still reachable from this branch?
    if ! git cat-file -e "$LAST_SHA" 2>/dev/null; then
      echo "lore: history has been rewritten since the last learn (rebase or force-push). Run /lore:refresh to recompute the baseline."
      exit 0
    fi

    COMMITS=$(git rev-list --count "$LAST_SHA"..HEAD 2>/dev/null || echo 0)

    # Loud signal: many commits since last learn.
    if [ "${COMMITS:-0}" -ge "$THRESH_COMMITS" ]; then
      echo "lore: $COMMITS commits since last learn. Run /lore:refresh to update CLAUDE.md and memory."
      exit 0
    fi

    # Even with few commits, a manifest change is high-signal.
    ...
    if [ -n "$CHANGED" ]; then
      for wf in $WATCH_FILES; do
        if printf '%s\n' "$CHANGED" | grep -Fxq "$wf"; then
          echo "lore: $wf changed since last learn. Run /lore:refresh to update."
          exit 0
        fi
      done
    fi
  ...
  if [ -n "$LAST_DATE" ]; then
    ...
    if [ -n "$THEN" ]; then
      ...
      if [ "$DAYS" -ge "$THRESH_DAYS" ]; then
        echo "lore: $DAYS days since last learn. Run /lore:refresh to update."
        exit 0
      fi
    fi
  fi
...
# =================================================================
# BRANCH B — no state file: should we suggest /lore:learn?
...
if [ "${COMMITS:-0}" -lt "$ONBOARD_MIN_COMMITS" ]; then
  exit 0
fi
...
# All conditions met: nudge once. (One line; the model decides whether to act.)
if [ -f CLAUDE.md ] || [ -f .claude/CLAUDE.md ]; then
  echo "lore: CLAUDE.md exists but lore has not learned this repo. Run /lore:learn to add a managed section and seed memory."
else
  echo "lore: this repo has no CLAUDE.md. Run /lore:learn to onboard it (analyzes the repo, writes CLAUDE.md, seeds memory)."
fi
exit 0
